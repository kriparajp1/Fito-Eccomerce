...
const getAddProduct = async (req, res) => {
    try {
        ...
        console.log(error);
        return res.render('admin/addProduct', { categories, error,menu:"products" });
    }
}
...
const getEditProduct = async (req, res) => {
    try {
        if (req.session.admin) {
            ...
            const product = await productDatabase.findById(proId).populate('product_category');

            const errors = req.query.errors || "";  // Retrieves error message from the query parameter

            if (product) {
                res.render('admin/editProduct', { product, categories, errors,menu:"product" });
            } else {
...
const postEditProduct = async (req, res) => {
    try {
        ...
        if (error.code === 11000 && error.keyPattern && error.keyValue) { // 11000: MongoDB error code for duplicate key
            const duplicateKeyName = Object.keys(error.keyPattern)[0];
            const duplicateKeyValue = error.keyValue[duplicateKeyName];
            res.redirect(`/admin/editProduct/${req.params.proId}?errors=Product+with+name+'${duplicateKeyValue}'+already+exists`);
        } else {
            console.log(error);
            res.redirect(`/admin/editProduct/${req.params.proId}?errors=An+error+occurred+during+editing+the+product`);
        }
    }
};
